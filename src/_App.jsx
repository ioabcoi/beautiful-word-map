...
const api = {
  getWords:    ()         => fetch(`${API}/words`).then(r => r.json()),
  addWord:     (word)     => fetch(`${API}/words`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(word) }).then(r => r.json()),
  updateWord:  (id, word) => fetch(`${API}/words/${id}`, { method:"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify(word) }).then(r => r.json()),
  deleteWord:  (id)       => fetch(`${API}/words/${id}`, { method:"DELETE" }).then(r => r.json()),
  getThemes:   ()         => fetch(`${API}/themes`).then(r => r.json()),
  addTheme:    (theme)    => fetch(`${API}/themes`, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(theme) }).then(r => r.json()),
  deleteTheme: (id)       => fetch(`${API}/themes/${id}`, { method:"DELETE" }).then(r => r.json()),
};
...
const WordForm = ({ initial, themes, onSave, onClose }) => {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(30,18,8,0.48)", backdropFilter:"blur(4px)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={onClose}>
...
export default function App() {
  ...
  const handleSaveThemes = async (newList, oldList) => {
    // 추가된 테마
    for (const t of newList) {
      if (!oldList.find(o => o.id === t.id)) await api.addTheme(t);
    }
    // 삭제된 테마
  ...
  return (
    <>
      ...
      {showThemeMgr && <ThemeManager themes={themes} onSave={(newList) => handleSaveThemes(newList, themes)} onClose={() => setTMgr(false)} />}
    </>
  );
}